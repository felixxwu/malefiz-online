...
function Icon(props: { pathString: string; size?: number; colour?: string; opacity?: number }) {
  const { pathString, size = 12, colour = colours.background, opacity } = props
...
function createIcon(pathString: string) {
  return (props: { size?: number; colour?: string; opacity?: number }) => {
    const { size, colour, opacity } = props
    return <Icon pathString={pathString} size={size} colour={colour} opacity={opacity} />
  }
}
...
export const PlayIcon = createIcon('M3 22v-20l18 10-18 10z')
